...
class NmapScanner:
    ...
    def quick_scan(self):
        ...
        cmd = f"nmap -sC -sV -T4 --open --stats-every 1s -oN {self.output_dir}/nmap_quick.txt -oX {self.output_dir}/nmap_quick.xml {self.target}"
    ...
    def full_port_scan(self):
        ...
        cmd = f"nmap -T4 --max-retries 1 --min-rate=1000 -p- --stats-every 1s -oN {self.output_dir}/nmap_all_ports.txt {self.target}"
        ...
        cmd = f"nmap -sC -sV -T4 --stats-every 1s -p {ports_str} -oN {self.output_dir}/nmap_services.txt -oX {self.output_dir}/nmap_services.xml {self.target}"
        output, _ = self._run_with_progress(cmd, "")
    ...
    def udp_scan(self, top_ports=200):
        """Scan top UDP ports"""
        cmd = f"nmap -sU --top-ports {top_ports} -T4 --stats-every 1s -oN {self.output_dir}/nmap_udp.txt {self.target}"
    ...
    def vulnerability_scan(self, open_ports=None):
        ...
        if open_ports:
            ports_str = ','.join(open_ports)
            cmd = f"nmap --script vuln --stats-every 1s -p {ports_str} -oN {self.output_dir}/nmap_vuln.txt {self.target}"
        else:
            cmd = f"nmap --script vuln --stats-every 1s -oN {self.output_dir}/nmap_vuln.txt {self.target}"
